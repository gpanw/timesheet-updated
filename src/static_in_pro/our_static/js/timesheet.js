...
viewTimesheet = {
	  ...
	  init: function () {
        ...
        $('#myModal').modal('show')
	       ...
	       $('.leave-item').click(function () {
	          ...
	          taskVal = {
        taskname: projectid + project_subtask,
	       ...
	       $('.last-timesheet').click(function () {
		      // $('.last-timesheet-div').remove();
		      selectedWeek = $('.selected').text().replace('Current', '')
		      curr = new Date(selectedWeek)
	              x = curr.getDate() - 7
    ...
    var i = 1
	  ...
	  getWeekList: function () {
	       ...
	       var append_week = "<p class='future-week selected'>" + add_week + ' Current</p>'
	  ...
	  appendTimeSheet: function (taskVal, hours) {
	        ...
	        appendVal += "<td><input class='hours sheetsat form-control' type='text'  value =" + hours[0] + '></td>'
	  ...
	  handle_addTimeSheet: function (response) {
	     if (response) {
	        if (response.rc == 1) {
	            ...
	            $('.save-status').html('Timesheet is saved')
	           } else {
